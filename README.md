...
Additional **packet parser** and **`/etc/services` parser** unit tests cover VLAN and cooked (**SLL**) frames, ICMP (no L4 ports), invalid IP versions, **truncated IPv4 or IPv6 payloads** (Ethernet, VLAN, **SLL**, **DLT_RAW**) shorter than the minimum IP header length, **BSD loopback** (**DLT_NULL**) including a minimal **AF_INET** ICMP frame, rejects unknown or zero address families (or **AF_INET** / **AF_INET6** with only the 4-byte family header and no IP payload — **AF_INET6** uses **10** on Linux and **30** on macOS), **CIDR** edge cases (including **`/0`**, prefix lengths beyond **`/32`** or **`/128`** behaving as exact-host match, IPv4 **`/17`** / **`/26`** / **`/28`** / **`/29`**, IPv6 **`/56`** link-local matching, **CLI** `-F` parsing for IPv6 nets such as **`fd00::/8`** and **`2001:db8::/32`**, **broadcast** **`255.255.255.255/32`**, **invalid** filters with **extra slashes**, an **empty prefix** after **`/`**, or **no** **`/`** at all, and **`/etc/services`** lines with **decimal ports** that include **leading zeros**, a **missing port token** (`ssh /tcp`), **commas inside the service name token**, or **blank-only** lines between entries, so regressions show up on **Ubuntu** and **macOS** without live capture hardware.

**Flow and history** unit tests cover **non-normalized** reversed **`FlowKey`** pairs as **distinct** tracker rows (until keys are canonicalized upstream), **same IPs with different ports** or **different IP protocols** (**TCP** vs **UDP** with identical endpoints) as **separate** flows, **recv-only** vs **send-only** sliding-window averages, **40s** sent and **recv** window sums with **eleven** completed slots, **40s** recv window parity with **2s** on a single slot, **IPv6-NoNxt** (**59**), **DCCP** (**33**), **RSVP** (**46**), **IPv6 encapsulation** (**41**), **HIP** (**139**), **L2TP** (**115**) as **`Protocol::Other`**, **`readable_size`** crossing from **b** into the **kb** tier when the bit rate exceeds **1000 b/s**, and **prefs** roundtrips for themes such as **SynthWave**, **PlasmaCore**, **NightCity**, **VoidWalker**, **Darkwave**, **Data Stream**, **Laser Grid**, and **Megacorp**.
