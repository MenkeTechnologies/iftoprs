...
Additional **packet parser** and **`/etc/services` parser** unit tests cover VLAN and cooked (**SLL**) frames, ICMP (no L4 ports), invalid IP versions, **BSD loopback** (**DLT_NULL**) rejects unknown or zero address families (or **AF_INET** / **AF_INET6** with only the 4-byte family header and no IP payload), and CIDR edge cases (including **`/0`**, which matches the full IPv4 or IPv6 space for same-family addresses) so regressions show up on **Ubuntu** and **macOS** without live capture hardware.
