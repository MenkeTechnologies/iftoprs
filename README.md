...
Additional **packet parser** and **`/etc/services` parser** unit tests cover VLAN and cooked (**SLL**) frames, ICMP (no L4 ports), invalid IP versions, **truncated IPv4 or IPv6 payloads** (Ethernet, VLAN, **SLL**, **DLT_RAW**) shorter than the minimum IP header length, **BSD loopback** (**DLT_NULL**) including a minimal **AF_INET** ICMP frame, rejects unknown or zero address families (or **AF_INET** / **AF_INET6** with only the 4-byte family header and no IP payload — **AF_INET6** uses **10** on Linux and **30** on macOS), **CIDR** edge cases (including **`/0`**, prefix lengths beyond **`/32`** or **`/128`** behaving as exact-host match, typical IPv4 **`/29`** matching, **CLI** `-F` parsing for IPv6 nets such as **`fd00::/8`** and **`2001:db8::/32`**, and **`/etc/services`** lines with **decimal ports** that include **leading zeros** or a **missing port token** (`ssh /tcp`), so regressions show up on **Ubuntu** and **macOS** without live capture hardware.

**Flow and history** unit tests cover **non-normalized** reversed **`FlowKey`** pairs as **distinct** tracker rows (until keys are canonicalized upstream), **recv-only** vs **send-only** sliding-window averages, **L2TP** (**115**) as **`Protocol::Other`**, and **`readable_size`** crossing from **b** into the **kb** tier when the bit rate exceeds **1000 b/s**.
